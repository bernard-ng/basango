...
export const articles = pgTable(
  "article",
  {
    ...
    tsv: tsvector("tsv").generatedAlwaysAs(
      sql`(setweight(to_tsvector('french', coalesce(title, '')), 'A') || setweight(to_tsvector('french', coalesce(body, '')), 'B'))`,
      { stored: true },
    ),
  },
  (table) => [
    ...
    index("idx_article_published_at")
      .using("btree", table.publishedAt.desc()),
    index("idx_article_published_id")
      .using("btree", table.publishedAt.desc(), table.id.desc()),
    unique("unq_article_hash").on(table.hash),
    index("gin_article_tsv").using("gin", table.tsv),
    index("gin_article_link_trgm").using(
      "gin",
      table.link.op("gin_trgm_ops"),
    ),
...
export const appUsers = pgTable(
  ...
  (table) => [
    uniqueIndex("unq_user_email").using(
      "btree",
